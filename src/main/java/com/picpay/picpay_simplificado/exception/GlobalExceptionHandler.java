...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponseDto> badRequestHandler(ClientBadRequestException ex) {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(new ErrorResponseDto(OffsetDateTime.now(),HttpStatus.BAD_REQUEST.toString(), ex.getMessage(), HttpStatus.BAD_REQUEST.value()));
    }

    @ExceptionHandler(UserNotFoundException.class)
    public ResponseEntity<ErrorResponseDto> userNotFoundHandler(UserNotFoundException ex) {
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(new ErrorResponseDto(OffsetDateTime.now(),HttpStatus.NOT_FOUND.toString(), ex.getMessage(), HttpStatus.NOT_FOUND.value()));
    }

    @ExceptionHandler(NotAuthorizedException.class)
    public ResponseEntity<ErrorResponseDto> notAuthorizedHandler(NotAuthorizedException ex){
        return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                .body(new ErrorResponseDto(OffsetDateTime.now(),HttpStatus.UNAUTHORIZED.toString(), ex.getMessage(), HttpStatus.UNAUTHORIZED.value()));
    }

    @ExceptionHandler(Exception.class)
    public ResponseEntity<ErrorResponseDto> globalErrorHandler(Exception ex) {
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(new ErrorResponseDto(OffsetDateTime.now(),"Unexpected error", ex.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR.value()));
    }
}
